...
- 6 jewellery = SilverRing / GoldRubyRing / SilverNecklace clones, Trinket,
  forge recipes off gems, Blacksmithing 15-50 craft gate.
...
- **AzuEPI custom slots**: `Custom Equipment Slots = Slot:Prefab,...` (sync).
  `m_itemType` must be Helmet/Chest/Legs/Shoulder/Utility/Trinket or it never
  equips. Slots merge by name; Wizardry owns `$azuepi_fingerslot`. Eitr needs
  `Slots Use Armor = On`. WIRSL `BlockEquip` fights AzuEPI - use `BlockCraft`.
