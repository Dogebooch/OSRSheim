...
### Generator
`gen-loot.py` writes the two `character_drop` cfgs, `gen-objects.py` writes
`drop_that.drop_table.cfg`, `gen-handbook.py` writes
`Dialogues\osrsheim_handbook.cfg` (bestiary: biome -> creature -> every drop at
its real chance, superior sub-pages; tinted by rarity per `TIERS`, `COLOUR =
False` removes them). Never hand-edit the output; all three `--check` in the
...
(creatures) / `.130+` (lists). `--literal`, `--wiring` and `--marker` leave the
cfgs in a test state; the validator warns until a plain run restores them.

Enforced: IDs >= 100 (vanilla drops at 0-2 never cleared); lists 110+/120+
keep their index; `ScaleByLevel = false`; <= 100 items per entry;
