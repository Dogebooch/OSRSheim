...
### Generator
`gen-loot.py` (two `drop_that` cfgs) and `gen-collection-log.py` (three KG
cfgs) write to the **profile**; copy them back to `config\` after a run.
...
Never hand-edit a generated cfg; all three `--check` in the validator.
...
(creatures) / `.130+` (lists). Flags: `--diff`, `--check`, `--literal` (multipliers
1), `--wiring` (all 100), `--marker <Creature> <Item>` (kill counter); the
