...
exceptions). Config audit: unfixed KG Talk quotes + buff multipliers (§11).
...
collection log (§11), fishing wave (§6, §11, §18), Sailing hull gates,
skilling pets + curios (§7), trinket + hatchet gates (§6), 7 elite uniques + crystal chest (§7, §11), hull keels +
7 OdinShip recipes (§6, §7, §11, §18), handbook + DistancedUI (§7, §11).
`drop_that.drop_table.cfg` is generated now (`gen-objects.py`); the old
weights dropped Feathers at 100% on ten empty log tables. Validator clean
bar drift: **another agent edits the profile directly** (13 clones with no
log row, 7 `.101` drops not in `drops.csv`). All four docs are at cap.
...
4. Watch for in play: superiors spawning, EpicLoot Haldor / map / bounty
   ...
   missing grey. Hulls (§6, §7): Hammer category, keel off a
...
## Server (later)
§15: profile export, host standup, `Use Marketplace Locally
= false`, clone-drop and `DropOnePerPlayer` checks with both players.
