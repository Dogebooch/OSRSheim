...
| Marlthon OdinShip | `marlthon.OdinShip.cfg` | `OSRS_Keel<Boss>:1:False` appended to the 7 hull `Crafting Costs`, War Ship takes Yagluth + Queen and no ShipRope (the build HUD fits 5 costs + the station; 6 threw every frame, #135); everything else default, BepInEx writes the full file on first launch | Sections by display name, apostrophes stripped (`[Merchants boat]`); costs `Prefab:amount:recover`. Ship mats from the Carpenters Table. PieceManager resolves costs at ObjectDB.Awake, before wackydb adds clones, and drops a missing item silently; re-resolves only on a `Crafting Costs` change. Hull costs restated in `wackysDatabase\Pieces\` (validator: must match). |
