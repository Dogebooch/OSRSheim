...
| File | Contents |
|---|---|
| `drop_that.character_drop.cfg` | generated: commons = coins + Gem tier; elites = coins + Rare tier; bosses = coin chunks + trophy + pet `.103` + riddle-stone `.105`; `unique=` rows omitted; every entry (and every list entry) `ConditionNotCreatureStates = Tamed` (validator-enforced) |
