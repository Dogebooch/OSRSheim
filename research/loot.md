...
| File | Contents |
|---|---|
| `drop_that.character_drop.cfg` | generated: commons = coins + Gem tier; elites = coins + Rare tier; bosses = pet `.103` + keel `.104`, no coins, no stone (the story boss quest pays both); `unique=` rows omitted; every entry (and every list entry) `ConditionNotCreatureStates = Tamed` (validator-enforced) |
...
| `drop_that.character_drop.osrsheim_superiors.cfg` | generated by `scripts\update-superiors.py` (ROWS): `.200+` coin chunks, `.210` gem, `.211` material, `.212` extra, `.213` rare on the 8 superiors (level 3 + boss key + not tamed); Troll `.200-.210` purse 100-200c + Ruby on the 510 troll only (`ConditionTemplateId = osrsheim_wanderer`) |
...
| Story boss quest, once per player | T1 Eikthyr, Elder · T2 Bonemass, Moder · T3 Yagluth, Queen · T4 Fader, Kall | |
| `slayer_boss_*` re-kill / `boss_bane_*` (kc 10, one tier above the boss, cap T4) | T2 Eikthyr, Elder · T3 Bonemass, Moder · T4 rest | |
...
| 510 | Wandering Meadows night troll, 1.4% per 1200 s, 1 roll a stale zone (~0.6/hr of nights), >300 m from center, no HuntPlayer, level 1 + `UseDefaultLevels = true` (no CLLC stars), `TemplateId = osrsheim_wanderer` (its `Troll.200/201/210` purse keys on it, not the biome) | Meadows, `defeated_eikthyr` |
| 511 | Fuling scouts, 2-3, night only, 0.5% per 1200 s, 3 rolls a stale zone (~0.6 groups/hr of nights), >500 m from center, level 1 + `UseDefaultLevels = true` | Black Forest, `defeated_dragon` (Plains loot, BlackMetalScrap) |
